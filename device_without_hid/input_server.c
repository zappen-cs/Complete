...
int left_flag = 0;
int right_flag = 0;
int boundary_shift = 6;
int left_button_pressed = 0;
int has_drag = 0;
int need_reopen_left = 0;
int need_reopen_right = 0;
char home_path[10086];
char has_drag_fifo_path[10086];
char enter_fifo_path[10086];
char release_fifo_path[10086];
...
#define NO_DND_PERFORMING 0x456
...
static void start_py(){
	DEBUG_INFO("Starting four.py\n");
	char cmd[10086];
	strcpy(cmd, "python3 four.py ");
	strcat(cmd, args);
	printf("%s", cmd);
	system(cmd);
}
...
static void handle_has_drag(){
	char has_drag_buf[100];
	if(access(has_drag_fifo_path, F_OK) != 0){
		int ret = mkfifo(has_drag_fifo_path, 0666);
		if (ret != 0){
			perror("Failed to create fifo");
		}
	}
	FILE* has_drag_fifo = fopen(has_drag_fifo_path, "r");
	if (has_drag_fifo == NULL){
		perror("Failed to open fifo");
	}
	fgets(has_drag_buf, 100, has_drag_fifo);
	has_drag = 1;
	fclose(has_drag_fifo);
}
...
static void handle_reaching_boundary(){
	...
	if (ret == 0) {
	    DEBUG_INFO("create set_released_thread successfully");
	}
	else{
	    DEBUG_INFO("Failed to create set_released_thread");
	}
...
static void set_released_func(){
	while(1){
		DEBUG_INFO("waiting for sem...");
		sem_wait(&sem);
		DEBUG_INFO("sending release....");
		report_event(EV_KEY, BTN_LEFT, 0);
		report_event(EV_SYN, SYN_REPORT, 0);
		handle_released_in_boundary();
	}
}
...
static void handle_event(struct libinput_event *event) {
	...
	if (libinput_event_get_type(event) == LIBINPUT_EVENT_POINTER_BUTTON) {
		...
		uint32_t current_button = libinput_event_pointer_get_button(pointer_event);
 		enum libinput_button_state state = libinput_event_pointer_get_button_state(pointer_event);
		
		if (current_button == BTN_LEFT){
			if (state == LIBINPUT_BUTTON_STATE_PRESSED && !left_button_pressed) {
				DEBUG_INFO("Mouse pressing the left button");
				left_button_pressed = 1;
			}
			else if (state == LIBINPUT_BUTTON_STATE_RELEASED && left_button_pressed) {
				DEBUG_INFO("Mouse releasing the left button");
...
#if ENABLE_DND
		// right boundary
		if (global_x > g_screen_width - buf_width && !right_flag && left_button_pressed) {
			DEBUG_INFO("Mouse reaching the right boundary");
			handle_reaching_boundary();
			right_flag = 1;
		}
		if ((global_x < g_screen_width - buf_width) && right_flag){
			DEBUG_INFO("Mouse leaving the right boundary or leftbtn released");
			handle_leaving_boundary();
			right_flag = 0;
		}

		// left boundary
		if (global_x < buf_width && !left_flag && left_button_pressed) {
			DEBUG_INFO("Mouse reaching the left boundary");
			handle_reaching_boundary();
			left_flag = 1;
		}
		if ((global_x > buf_width) && left_flag) {
			DEBUG_INFO("Mouse leaving the left boundary or leftbtn released");
			handle_leaving_boundary();
			left_flag = 0;
		}
#endif
		...
		if (global_x < 0) {
			...
			if (g_device_seat == DEVICE_SEAT_RIGHT) {
				...
				if(left_button_pressed){
					...
					if (has_drag){
						DEBUG_INFO("Dragging");
						mke.dnd_status = MOUSE_LEFT_BTN_PRESSED;
						has_drag = 0;
					}
					else{
						DEBUG_INFO("No dragging");
						mke.dnd_status = NO_DND_PERFORMING;
					}
				}else if(!left_flag){
					handle_reaching_boundary();
					left_flag = 1;
...
#if ENABLE_DND
		int g_screen_width_adjusted = g_screen_width;
		if (left_button_pressed && DEVICE_SEAT_LEFT)
			g_screen_width_adjusted -= boundary_shift;
		if (global_x > g_screen_width_adjusted) {
#endif
#if !ENABLE_DND
	...
	while (1) {
		...
		while (1) {
			...
			if (ret <= 0) { 
				...
				if (mke.type == SET_MOUSE_POSITION) { // First arrival
					...
					if (g_device_seat == DEVICE_SEAT_UP) {
						...
						global_x = g_screen_width;
#if ENABLE_DND
						if(mke.dnd_status == MOUSE_LEFT_BTN_PRESSED){
							global_x -= boundary_shift;
						}
#endif
					...
					set_mouse_position(global_x, global_y);
#if ENABLE_DND
					if (mke.dnd_status == NO_DND_PERFORMING){
						DEBUG_INFO("No DND Performing");
						send_close_signal();
						right_flag = 0;
						left_flag = 0;
					}
					else if(mke.dnd_status == MOUSE_LEFT_BTN_PRESSED){
						DEBUG_INFO("sending BTN_LEFT pressed....");
						report_event(EV_KEY, BTN_LEFT, 1);
						report_event(EV_SYN, SYN_REPORT, 0);
						// send_minimize_signal();
					}
#endif
...
#if ENABLE_DND
	// g_screen_width -= boundary_shift;
	strcpy(args, "-place edge");

	const char *username = getlogin();
	strcpy(home_path, "/home/");
	strcat(home_path, username);
	DEBUG_INFO("home_path: %s", home_path);

	strcpy(has_drag_fifo_path, home_path);
	strcat(has_drag_fifo_path, "/.hasdragfifo");
	strcpy(enter_fifo_path, home_path);
	strcat(enter_fifo_path, "/.enterfifo");
	strcpy(release_fifo_path, home_path);
	strcat(release_fifo_path, "/.releasefifo");
	strcpy(minimize_fifo_path, home_path);
	strcat(minimize_fifo_path, "/.minimizefifo");
...
#if ENABLE_DND
	// keep a surface on the top
	handle_reaching_boundary();
	if (DEVICE_SEAT_RIGHT){
		left_flag = 1;
	}
	else if(DEVICE_SEAT_LEFT){
		right_flag = 1;
	}
#endif
