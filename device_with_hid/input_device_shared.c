...
static void handle_event(struct libinput_event *event) {
	...
	if (libinput_event_get_type(event) == LIBINPUT_EVENT_POINTER_MOTION) {
		...
		if (global_x > g_screen_width - buf_width && !right_flag && left_button_pressed) {
			DEBUG_INFO("Mouse reaching the right boundary, creating surface");
		...
		if (global_x < buf_width && !left_flag && left_button_pressed) {
			DEBUG_INFO("Mouse reaching the left boundary, creating surface");
		...
		if (global_x <= 0) {
			...
			if (left_device_index >= 0) {
				...
				if(left_button_pressed){
					...
					strcpy(args, "-target left");
					DEBUG_INFO("Mouse leaving current computer, creating surface");
					handle_reaching_boundary();
					left_flag = 1;
					mke.dnd_status = NO_DND_PERFORMING;
				}
		...
		if (left_button_pressed && right_device_index >= 0)
