...
static void start_py(){
	...
	system("python3 four.py");
}


static void handle_has_drag(){
	char has_drag_buf[100];
	if(access("/home/tclab/hasdragfifo", F_OK) != 0){
		int ret = mkfifo("/home/tclab/hasdragfifo", 0666);
		if (ret != 0){
			perror("Failed to create fifo");
		}
	}
	FILE* has_drag_fifo = fopen("/home/tclab/hasdragfifo", "r");
...
static void handle_leaving_boundary(){
	FILE *fifo = fopen("/home/tclab/enterfifo", "w");
	if (fifo == NULL){
		perror("Failed to open fifo");
	}
...
static void handle_released_in_boundary(){
	FILE *fifo = fopen("/home/tclab/releasefifo", "w");
	if (fifo == NULL){
		perror("Failed to open fifo");
	}
	DEBUG_INFO("FIFO WRITTEN");
...
static void send_minimize_signal(){
	FILE *fifo = fopen("/home/tclab/minimizefifo", "w");
	if (fifo == NULL){
		perror("Failed to open fifo");
	}
	DEBUG_INFO("FIFO WRITTEN");
...
static void send_close_signal(){
	FILE *fifo = fopen("/home/tclab/closefifo", "w");
	if (fifo == NULL){
		perror("Failed to open fifo");
	}
	DEBUG_INFO("FIFO WRITTEN");
